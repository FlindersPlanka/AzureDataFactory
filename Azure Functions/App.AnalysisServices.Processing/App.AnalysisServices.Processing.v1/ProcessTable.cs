...
namespace App.AnalysisServices.Processing.v1
{
    public static class ProcessTable
    {
        ...
        public static async Task<HttpResponseMessage> Run([HttpTrigger(AuthorizationLevel.Anonymous, "get", "post", Route = null)]HttpRequestMessage req, TraceWriter log, ExecutionContext context)
        {
            ...
            // Read the Connection String of the Analysis Services instance
            var connstring = ConfigurationManager.AppSettings["SqlConnectionString"];
            ...
            //string connString = config.GetConnectionString("SqlConnectionString");

            // Read the Query String
            string maxParallelism = req.Query["maxParallelism"];                                // 5
            string ssasFunction = req.Query["ssasFunction"];                                    // "MergePartitions"
            string ssasDatabaseConnectionString = req.Query["ssasDatabaseConnectionString"];    // "Data Source=devsqlolap01\tabular;Initial Catalog=CreditRisk_PartitionTest;Provider=MSOLAP.8;Integrated Security=SSPI;Impersonation Level=Impersonate;";
